...
from utils.model_util import create_diffusion, create_karras_denoiser

from inference.models.cm.karras_diffusion import stochastic_iterative_sampler
...
def train(epoch, net, diffusion, schedule_sampler, device, train_data, optimizer, batches_per_epoch, denoiser, vis=False):
    """
    ...
    while batch_idx <= batches_per_epoch:
        for x, y, _, _, _, prompt, query in train_data:
            ...
            loss = denoiser.consistency_losses(model=net, x_start=yc)
            print(loss)
...
def run():
    ...
    # Load the Karras denoiser
    denoiser = create_karras_denoiser()
    ...
    for epoch in range(args.epochs):
        ...
        train_results = train(epoch, net, diffusion, schedule_sampler, device, train_data, optimizer, args.batches_per_epoch, denoiser, vis=args.vis)
